...
// ✅ Route: Like/Unlike a contribution
router.post("/:id/like", verifyToken, async (req, res) => {
  try {
    const contribution = await Community.findById(req.params.id);
    if (!contribution) return res.status(404).json({ message: "Contribution not found" });
    
    const userId = req.user.id;
    const userLikedIndex = contribution.likes.indexOf(userId);
    
    if (userLikedIndex > -1) {
      // User already liked, so unlike
      contribution.likes.splice(userLikedIndex, 1);
    } else {
    ...
    res.json({ 
      ...
      liked: userLikedIndex === -1 // true if just liked, false if just unliked
    });
  } catch (error) {
    res.status(500).json({ message: "Error liking contribution" });
  }
});

// ✅ Route: Add comment to a contribution
router.post("/:id/comment", verifyToken, async (req, res) => {
  try {
    ...
    res.json({ comments: contribution.comments });
  } catch (error) {
    res.status(500).json({ message: "Error adding comment" });
  }
});

// ✅ Route: Get comments for a contribution
...
router.post("/:id/share", async (req, res) => {
  try {
    ...
    res.json({ shares: contribution.shares });
  } catch (error) {
    res.status(500).json({ message: "Error sharing contribution" });
  }
});
